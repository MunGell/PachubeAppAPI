<?php
/**
 * Pachube Application Feed API class
...
class PachubeFeedAPI
{
	 ...
	 */
	public function updateFeed($format=false, $data)
	{
		$url = "http://$this->Pachube/feeds/$this->Feed";
		if($format && ($format == "json" || $format == "csv" || $format == "xml")) $url .= ".". $format;
	 ...
	 */
	public function getDatastreamsList()
	{
		$url = "http://$this->Pachube/feeds/$this->Feed.json";
	...
	/**
	 * Create datastream
	 * @param string datastream name, first value
	 * @return http response headers
	 */
	public function createDatastream($format=false, $data)
	{
		$url = "http://$this->Pachube/feeds/$this->Feed/datastreams";
		if($format && ($format == "json" || $format == "csv" || $format == "xml")) $url .= ".". $format;
	 ...
	 * @return http response headers
	 */
	public function deleteDatastream($datastream)
	{
		$url = "http://$this->Pachube/feeds/$this->Feed/datastreams/$datastream";
		return $this->_deleteRequest($url);
	}
	 ...
	 * @return string
	 */
	public function getDatastreamHistory($format, $datastream, $start=false, $end=false, $duration=false, $page=false, $per_page=false, $time=false, $find_previous=false, $interval_type=false, $interval=false)
	{
		if($format && ($format == "json" || $format == "csv" || $format == "xml")) $datastream .= ".". $format;
		$url = "http://$this->Pachube/feeds/$this->Feed/datastreams/$datastream?";
		if($start) $url .= "start=" . $start . "&";
